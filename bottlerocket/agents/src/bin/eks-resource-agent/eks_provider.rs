...
impl ClusterConfig {
    pub fn new(
        ...
        version: &Option<K8sVersion>,
        zones: &Option<Vec<String>>,
        encoded_config: &Option<String>,
    ) -> ProviderResult<Self> {
        let config = match (cluster_name, encoded_config) {
            (_, Some(eksctl_config)) => {
            ...
            (Some(cluster_name), None) => Self::Args {
            ...
            (None, None) => {
                return Err(ProviderError::new_with_context(
                    Resources::Clear,
                    "A cluster name or and eksctl config must be provided.",
                ));
            }
        };
        Ok(config)
